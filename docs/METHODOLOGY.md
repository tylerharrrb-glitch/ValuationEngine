...
- Comparables: peers entered as raw data (price, shares, debt, cash, minority interest, revenue, EBITDA, net income, book equity, source and date per peer). Peer EV = price × shares + debt − cash + minority interest. Multiples are computed (a non-positive denominator gives no multiple); median, mean and quartiles (linear interpolation). Implied values use medians: EV/EBITDA on normalized base-year EBITDA and on year-1 EBITDA (through the bridge of 5.3), P/E on reported base-year and year-1 net profit, P/B on book equity, EV/revenue. The comparables value used in the blend is the median EV/EBITDA (base year) implied value.
...
- Credit metrics: net debt / EBITDA (debt including leases, less unrestricted cash and investments, over reported EBITDA), interest cover (reported operating profit ÷ debt and lease interest), FFO / debt with FFO = reported EBITDA + finance income − debt and lease interest − current tax.
- FX sensitivity: value per share when USD/EGP moves by m ∈ {−20%, −10%, +10%, +20%} as a permanent level shift from year 1. Revenue × (1 + s_R × m) and cash costs (cash cost of sales and SG&A) × (1 + s_C × m), where s_R and s_C are the USD-linked shares of revenue and costs (user inputs; MOPCO export share 79.97% pre-filled as a sourced fact, USD cost share has no default). Capex and working capital follow the shocked revenue and costs; depreciation of existing PP&E, finance income and the balance-sheet bridge are unchanged (limitation: USD-denominated financial assets are not revalued).
- Piotroski variant: ratios use year-end total assets because the balance sheet before the first historical year is not available.
- LBO: not provided (see docs/PROGRESS.md, decision on secondary modules).
